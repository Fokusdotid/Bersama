...
END:VCARD
  ...
  conn.sendContactS = async (jid, number, name, quoted, options) => {
	const vcard = `BEGIN:VCARD
VERSION:3.0
N:;;;;
FN:${name}
item1.X-ABLabel:👤 Gada pawang nih senggol dong 😣
URL;Web:🔥 http://raselcomel.github.io
EMAIL;📧 Email:✨ raselcomel18@gmail.com
ORG: 👑 creator
item4.ADR:;;🇮🇩 Indonesia;;;;
item4.X-ABADR:ac
item4.X-ABLabel:🌍 Region
TEL;🚫 don't spam/call;waid= ${number}:${number}
END:VCARD`
...
`.trim()
    ...
    conn.trans = async(code, teks) => { 
        let tr = require('translate-google-api') 
        return tr(teks, { from: 'id', to: code })
    }
    /**
    *status 
    */
   conn.setBio = async (status) => {
    ...
    text: text, jpegThumbnail: await(await fetch(thumbr1)).buffer(), mentions: men}, {quoted: quoted})
    }
    conn.reply2 = async (jid, text, media, quoted, men) => {
    return conn.sendMessage(jid, {
    text: text, jpegThumbnail: await(await fetch(media)).buffer(), mentions: men}, {quoted: quoted})
    }
